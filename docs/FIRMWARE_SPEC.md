...
| Brownout hold, no evidence (`kInvalidReadsBeforeInhibit` = 4 unreadable cycles) | Uplinks, flash writes | Any valid reading | **No**, once bounded — but it *would* be terminal unbounded, since the pack may be full and the link simply broken | `kNoEvidenceKeepaliveCycles` = 24 held cycles, then one uplink regardless (#45). Transitioning an existing hold from measured evidence to no evidence changes permission without restarting the clock, so intermittent validity cannot postpone it forever |
...
| **Frame-counter ceiling refusal** | Every uplink | A successful checkpoint, proven removal, or safe filesystem rebuild | **Yes — this was the terminal one.** The original exit was the write that was failing. In uninterrupted runtime the boundary arrives every 32 uplinks (~8 h at 900 s); after reset the restored counter consumes the reserve, so the first uplink checkpoints before transmitting | Three failures, then remove the stored session. If removal returns an I/O error while the pack is safe, format, rewrite Config, and re-anchor the live session; a failed format retries after 96 eligible calls, not every wake. During a brownout hold, never format: sparse keepalives retry the small operation and the first healthy cycle repairs |
| **Fresh-join deferral** (`s_fresh_join_blocked`) | Every uplink, by refusing the join | A stored session that restores, or proof the stale one is gone | **Yes — this was the second terminal one.** Removing the stale session is a flash write, and the brownout gate forbids flash writes for the whole duration of the hold the keepalive is supposed to bound. A persisted hold plus a stored session that will not restore left the node mute and, being Class A, uncommandable, with no exit at all if the one-wire link was the thing that failed | An armed keepalive authorizes one join attempt (`session::permit_join_escape()`), which first spends one bounded `lfs_remove` — the same size of write the counter checkpoint already takes during a hold, and deliberately not a format. If the removal lands the join is wholly safe; if it fails the node joins anyway and accepts that a reset before the next repair may resume an invalidated session. Ordinary cycles stay deferred, so the bound is the keepalive cadence itself |
...
Boot mounts the filesystem **non-destructively**. Adafruit's
`InternalFileSystem::begin()` erases all seven filesystem pages and formats after one failed
mount ([CITE(prior-art): InternalFileSystem source — `CIT-INTERNAL-FS`](CITATIONS.md)); it is
therefore forbidden before the first pack reading and the brownout gate. A failed boot mount
runs on defaults and leaves flash intact for the same gated repair path above.
