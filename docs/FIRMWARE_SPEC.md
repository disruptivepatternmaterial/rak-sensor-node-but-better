...
That prohibition governs a build whose console **is** initialized, and it still stands there unchanged. The field image takes the other route: it never initializes the port at all. RAK's low-power example states that the serial port "**MUST NOT** be initialized" because FreeRTOS starts a background task for it that never sleeps and so prevents the MCU from sleeping, and it enforces that by wrapping `Serial.begin()` itself in a compile switch ([CITE(prior-art): RAK4630 LoRaWAN Low Power Example, line 45](https://github.com/RAKWireless/WisBlock/blob/master/examples/RAK4630/communications/LoRa/LoRaWAN/Low_Power_Example.md)). So `env:rak4631` builds `FEATURE_CONSOLE=0` and never reaches `Serial.begin()`, while `env:soak` builds the same image with the console in and is the environment every bench observation is taken on. The two must differ in that flag and nothing else — an observation is only evidence about the field image while that holds. The predicted saving is milliamps against a microamp budget and is **not yet measured on this node** (issue #47).
