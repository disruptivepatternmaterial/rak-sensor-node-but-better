...
**Leading standard replacement: Pololu `D24V22F5`, not yet adopted or connected.** It accepts
5.3–36 V, supplies regulated 5 V with ample margin for this board, draws approximately 1 mA at
no load, and has documented soft-start plus input reverse-voltage, over-current, short-circuit,
and thermal protection [CIT-POLOLU-D24V22F5]. Unlike the two potted converters, its relevant
behavior is published. It is a bare 0.7-inch module for installation inside the existing
enclosure, not a native USB-C product.

This choice can reduce and make input inrush repeatable; it cannot certify the one-wire line.
Both bucks tested failed the mating capture the same way, so the buck is not the data-line fix
and is chosen on power grounds only.
