...
Documentation only; no firmware change. **🚧 NOT YET DEPLOYED.**
...
- **ADR-0010 closed by the drawings.** The RAK4631 schematic wires connector pins 17/18 (`VDD`)
  to the base board's own `3V3`; the "generated by MCU module" wording and the "base board has a
  regulator" reports were both true of different nets. A coreless `VDD` pad is open-circuit, not
  a disputed source.
- **`CIT-RAK19007-DS` claimed no over-voltage clamp on USB-C.** The power sheet shows `D3`, a
  5.6 V / 500 mW Zener across `VBUS_D`. The 5.5 V ceiling stands; the sentence was wrong.
- **`docs/CITATIONS.md` gains 21 rows** — every RAK schematic this node uses, read as an image
  with reference designators and values recorded (RAK4631, RAK19007 ×2, RAK5802, RAK9154 block
  diagram, RAK19010, RAK19016 ×2, Weather Station manual with the SP11 part numbers, Adafruit
  bootloader `REGOUT0`), and one row per component named on them (`TP8485E`, `LRC399-04AT1G`,
  `SGM6036`, `SGM61230`, `TP4054`, `NCE40P05Y`, `MBR140SFT1G`, `LBZT52B5V6T1G`, `DTC043ZEBTL`,
  Weipu SP11). The `SDA` clip's 4.7 kΩ base-board pull-up and the RAK5802's NC `TCON` link to
  `IO1` came out of this.

### Open check surfaced

- The RAK4631's nRF52840 is powered from `VBAT` (`VDDH`); its GPIO rail is `UICR.REGOUT0`,
  default 1.8 V unless the bootloader wrote 3.3 V. Unread on every core this project owns; a
  non-destructive SWD read at `0x10001304`. `docs/HARDWARE.md` § "What the schematics establish".
